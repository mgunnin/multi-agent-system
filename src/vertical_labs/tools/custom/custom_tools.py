...
class CustomFileWriteTool(BaseTool):
    ...
    def _get_full_path(self, filename: Optional[str]) -> str:
        ...
        if not full_path.startswith(os.path.abspath(self._base_folder)):
            raise ValueError(
                "Access outside the base directory is not allowed."
            )  # TODO: add validations for path traversal

        return full_path

    def _run(
        self, content: str, mode: str, filename: Optional[str] = None
    ) -> Dict[str, Any]:
...
class CustomApiToolInputSchema(BaseModel):
    ...
    headers: Optional[Dict[str, str]] = Field(
        None, description="HTTP headers to include in the request"
    )
    query_params: Optional[Dict[str, Any]] = Field(
        None, description="Query parameters for the request"
    )
    body: Optional[Dict[str, Any]] = Field(
        None, description="Body of the request for POST/PUT methods"
    )
...
class CustomCodeInterpreterTool(BaseTool):
    ...
    description: str = (
        "Interprets Python3 code strings with a final print statement. Requires eighter code or run_script to be provided."
    )
        ...
        if libraries and len(libraries) > 0:
            for library in libraries.split(","):
                ...
                if install_result.exit_code != 0:
                    print(
                        f"Something went wrong while installing the library: {library}"
                    )

...
from .agents import (create_brand_analyst, create_media_relations_specialist,
                     create_pitch_coordinator, create_pitch_writer)
...
def create_brand_analysis_task(agents: Dict[str, Agent], context: Dict) -> Task:
    ...
    return Task(
        description=(
            "Analyze the brand's fit with available topics and publishers using "
            "the BrandMatchingTool. Consider brand values, target market, and "
            "unique selling propositions."
        ),
        expected_output=(
            "A prioritized list of brand-topic-publisher matches with "
...
def create_pitch_writing_task(agents: Dict[str, Agent], context: Dict) -> Task:
    ...
    return Task(
        description=(
            "Create compelling PR pitches for the high-priority matches using "
            "PitchGeneratorTool. Ensure each pitch highlights the unique value "
            "proposition and aligns with the topic."
        ),
        expected_output=(
            "A set of initial PR pitches with subject lines, body content, "
            "and calls to action."
        ),
...
def create_pitch_optimization_task(agents: Dict[str, Agent], context: Dict) -> Task:
    ...
    return Task(
        description=(
            "Optimize each pitch using PitchOptimizationTool based on publisher "
            "preferences and historical data. Refine language, structure, and "
            "timing for maximum impact."
        ),
        expected_output=(
            "A set of optimized pitches with publisher-specific adjustments "
...
def create_final_review_task(agents: Dict[str, Agent], context: Dict) -> Task:
    ...
    return Task(
        description=(
            "Review and finalize all pitches ensuring quality, consistency, and "
            "strategic alignment. Prepare delivery schedule and tracking system."
        ),
        expected_output=(
            "Final pitch package with delivery schedule and success metrics "
            "tracking plan."
        ),
